...
def validateStrategy(G1):
    ...
    if G1['r'].adj!=G2['r'].adj or G1['b'].adj!=G2['b'].adj:
        print(f'Error: the graph in the line {lineNum+1} does not match the expected graph')
        print(lineNum)
    ...
    return result
Gdict={
    'empty':{'r':nx.from_edgelist([]),'b':nx.from_edgelist([])},
    'b-path':{'r':nx.from_edgelist([]),'b':nx.from_edgelist(['01'])},
    'br-path':{'r':nx.from_edgelist(['12']),'b':nx.from_edgelist(['01'])},
    'brr-path':{'r':nx.from_edgelist(['12','23']),'b':nx.from_edgelist(['01'])},
    'brb-path':{'r':nx.from_edgelist(['12']),'b':nx.from_edgelist(['01','23'])},
    'd-multipath':{'r':nx.from_edgelist(['01']),'b':nx.from_edgelist(['01'])},
    'dr-multipath':{'r':nx.from_edgelist(['01','12']),'b':nx.from_edgelist(['01'])},
    'drr-multipath':{'r':nx.from_edgelist(['01','12','23']),'b':nx.from_edgelist(['01'])},
    'drb-multipath':{'r':nx.from_edgelist(['01','12']),'b':nx.from_edgelist(['01','23'])}
}
def validateFile():
